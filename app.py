...
tumor_size = st.sidebar.slider("肿瘤大小（毫米，以最大直径测量）", min_value=1, max_value=150, value=20, help="范围: 1-150")
time_to_response = st.sidebar.slider("确诊到治疗响应时间（天）", min_value=0, max_value=731, value=30, help="范围: 0-731天")

# 分类变量
st.sidebar.subheader("分类变量")
race = st.sidebar.selectbox("种族", 
                           options=[1, 2, 3], 
                           format_func=lambda x: {1: "白种人", 2: "黑种人", 3: "亚裔、太平洋岛屿原住民、美洲印第安人/阿拉斯加原住民"}[x])

marital_status = st.sidebar.selectbox("婚姻状况", 
                                     options=[1, 2, 3, 4], 
                                     format_func=lambda x: {1: "已婚/有稳定伴侣", 2: "离婚/分居", 3: "丧偶", 4: "从未结婚"}[x])

family_income = st.sidebar.selectbox("家庭中位收入（美元）", 
                                    options=[1, 2, 3, 4], 
                                    format_func=lambda x: {1: "0-60000", 2: "60000-80000", 3: "80000-100000", 4: "100000以上"}[x])
...
histology = st.sidebar.selectbox("组织学类型", 
                                options=[1, 2, 3], 
                                format_func=lambda x: {1: "导管癌 (8500-8508)", 2: "小叶癌 (8520-8524)", 3: "其他特殊类型"}[x])

grade = st.sidebar.selectbox("组织学分级", 
                            options=[1, 2, 3, 4], 
                            format_func=lambda x: {1: "高分化；I级", 2: "中分化；II级", 3: "低分化；III级", 4: "未分化；IV级"}[x])

er_status = st.sidebar.selectbox("ER状态", 
                                options=[0, 1], 
...
breast_subtype = st.sidebar.selectbox("乳腺亚型", 
                                     options=[1, 2, 3, 4], 
                                     format_func=lambda x: {1: "HR-/HER2-", 2: "HR-/HER2+", 3: "HR+/HER2-", 4: "HR+/HER2+"}[x])
...
bone_metastasis = st.sidebar.selectbox("骨转移", 
                                      options=[0, 1], 
                                      format_func=lambda x: {0: "否", 1: "是"}[x])

brain_metastasis = st.sidebar.selectbox("脑转移", 
                                       options=[0, 1], 
                                       format_func=lambda x: {0: "否", 1: "是"}[x])
